...
def scalars_to_quad_edges(scalars: Strategy[Scalar]) -> Strategy[QuadEdge]:
...
def scalars_to_points_in_general_position(scalars: Strategy[Scalar],
                                          *,
                                          min_size: int = 2,
...
def scalars_to_quad_edges_with_neighbours(scalars: Strategy[Scalar]
                                          ) -> Strategy[QuadEdge]:
    return (scalars_to_points_in_general_position(scalars,
                                                  min_size=4)
            .map(triangular.delaunay)
